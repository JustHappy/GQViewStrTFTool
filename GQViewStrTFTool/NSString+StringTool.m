...
#pragma mark: 清楚字符串中的所有的空格
-(NSString *)trimString{
    return [self stringByReplacingOccurrencesOfString:@" " withString:@""];
}
#pragma mark =====判断是否为空字符串
-(BOOL)isEmptyString{
    NSString * str = [self trimString];
    return (str.length == 0 || self == nil);
}
#pragma mark ====判断是否为空字符串，并且判断是否含有非法字符串
-(BOOL)isEmptyStringOrIncludeSpecialCharact{
    NSString * str = [self trimString];
     NSRange urgentRange = [str rangeOfCharacterFromSet: [NSCharacterSet characterSetWithCharactersInString: @"~￥#&*<>《》()[]{}【】^@/￡¤￥|§¨「」『』￠￢￣~@#￥&*（）——+|《》$_€"]];
    return (str.length == 0 || self == nil || urgentRange.location != NSNotFound);
}

#pragma mark =====判断是否包含空格
-(BOOL)isContainSpace{
    NSRange range = [self rangeOfString:@" "];
    return range.location != NSNotFound;
}
#pragma mark === 判断当前字符串是否全为数字
-(BOOL)isAllNum{
    NSString * regEx = @"^-?\\d+.?\\d?";
    NSPredicate * pred = [NSPredicate predicateWithFormat: @"SELF MATCHES %@",regEx];
    return [pred evaluateWithObject:self];
}
#pragma mark =====判断当前字符串是否为邮箱
